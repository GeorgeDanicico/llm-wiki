# Sources

This directory preserves original input material. Source files are immutable after capture; corrections, interpretation, and synthesis belong under [`../wiki/`](../wiki/index.md).

## Imported collection

The initial collection was imported on 2026-08-11 from the local `gd/Notes` repository (referred to as `gd/Notex` in the import request).

| Source | Type | Compiled knowledge |
| --- | --- | --- |
| [`books/designing-data-intensive-applications.md`](books/designing-data-intensive-applications.md) | Personal book notes | [Book page](../wiki/library/books/designing-data-intensive-applications.md), [Distributed systems](../wiki/distributed-systems/index.md) |
| [`notes/technologies/docker.md`](notes/technologies/docker.md) | Personal technology note | [Dockerfile multi-stage builds](../wiki/infrastructure/dockerfile-multi-stage-builds.md) |
| [`notes/technologies/domain-name-server-dns.md`](notes/technologies/domain-name-server-dns.md) | Personal technology note | [DNS](../wiki/infrastructure/dns.md) |
| [`notes/technologies/quarkus.md`](notes/technologies/quarkus.md) | Empty personal technology note | Not compiled; retained as an empty placeholder |
| [`notes/technologies/token-buckets-and-network-bandwidth-shaping.md`](notes/technologies/token-buckets-and-network-bandwidth-shaping.md) | AI-assisted conversational note | [Token buckets and network bandwidth shaping](../wiki/infrastructure/token-buckets-and-network-bandwidth-shaping.md) |

## Later captures

| Source | Type | Captured | Compiled knowledge |
| --- | --- | --- | --- |
| [`articles/kafka-consumers-and-durability.md`](articles/kafka-consumers-and-durability.md) | Technical article | 2026-08-18 | [Library entry](../wiki/library/articles/kafka-consumers-and-durability.md), [Kafka consumer progress and durability](../wiki/distributed-systems/kafka-consumer-progress-and-durability.md) |
| [`notes/interview-preparation/java-spring-senior-interview-knowledge.md`](notes/interview-preparation/java-spring-senior-interview-knowledge.md) | Synthesized interview notes | 2026-08-29 | [Library entry](../wiki/library/notes/java-spring-senior-interview-knowledge.md), [Interview guide](../wiki/interview-preparation/senior-java-spring-interview-guide.md) |
| [`designs/payment-idempotency-design-summary.md`](designs/payment-idempotency-design-summary.md) | Design summary | 2026-08-17 | [Library entry](../wiki/library/designs/payment-idempotency-design-summary.md), [Payment idempotency and double-charge prevention](../wiki/distributed-systems/payment-idempotency-and-double-charge-prevention.md) |

The source text may be incomplete or inaccurate. A source's presence records what was captured, not that every statement has been externally verified.
